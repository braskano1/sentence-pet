...
> **Consent / privacy.** Out of scope for this slice by decision: this is a **school-internal game**; guardian/parental consent has been handled offline with the school and parents. No in-app PDPA notice, consent gate, or consent record is built. `SignUpForm` collects email + password only.

## Goal

Every player is a Firebase user from first load (anonymous), their state is mirrored to per-user Firestore docs via a debounced sync layer, and any student can upgrade to an email/password account that follows them across devices — all behind the existing owner-only security rule, with the player app newly wrapped in auth.
...
src/components/account/
  SignUpForm.tsx     // email + password (functional; impeccable polishes later)
  AccountButton.tsx  // guest: "Save your pets across devices" → SignUpForm; signed-in: email + Sign out
docs/
  firebase-setup.md  // EXTEND: enable Anonymous auth provider
```
...
- **`src/firebase/users.ts`** — the only module that knows the Firestore paths/shapes for user data. Exports: `loadCloudSave(uid) → {profile, pets[]} | null`, `saveProfile(uid, profileDoc)`, `savePet(uid, petDoc)`. Stamps `updatedAt: serverTimestamp()` and `persistVersion`. Depends on: `firebase/firestore`, `./db`.
...
2. Form collects email + password.
3. Submit → `linkEmailPassword(email, pw)` (anon account becomes an email account, **same uid, progress untouched**).
...
This grants the owner read+write across `profile` and `pets/*`, and denies everyone else by default. **Extend the rules-test matrix** (`src/firebase/rules.test.ts`) with concrete cases for the new docs:
- owner can read+write own `users/{uid}/profile` and `users/{uid}/pets/{petId}`;
...
- **Component render tests** — `SignUpForm` (renders email/password fields, submit calls `linkEmail`), `AccountButton` (guest vs signed-in states), `AuthProvider` anon-bootstrap — with `src/firebase/*` and/or `useAuth` mocked (existing `vi.mock` convention; mock `canvas-confetti` if transitively imported).
...
- ❌ In-app PDPA/consent UI or consent record — school-internal game, consent handled offline.
...
- A guest taps "Save your pets", signs up with email + password → `linkWithCredential` keeps the same uid and all progress.
- Signing in with that email on a second device overwrites local guest progress with the account's cloud save (cloud wins).
- Rules tests encode owner-allowed / non-owner-denied / unauth-denied for `profile` and `pets/*`.
